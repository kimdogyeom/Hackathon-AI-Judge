...
class AccessibilityChain(EvaluationChainBase):
    """
    접근성 평가 체인.
    NovaLiteLLM을 사용하여 프로젝트의 접근성을 평가합니다.
    """
    ...
    def _load_evaluation_criteria(self, config_path: str):
        try:
            criteria_data = ChainUtils.load_evaluation_criteria(config_path, 'Accessibility')
    ...
    def _analyze(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 접근성 평가 수행
        return self._evaluate_accessibility(project_info, project_type)

    def _evaluate_accessibility(self, project_info: str, project_type: str = "balanced") -> Dict[str, Any]:
        """
        NovaLiteLLM을 사용하여 접근성을 평가합니다.
    ...
    def _build_system_prompt(self, project_type: str = "balanced") -> str:
        ...
        return f"""당신은 접근성 평가 전문가입니다. 이미 분류된 프로젝트 유형({project_type})을 고려하여 서비스의 접근성을 평가해주세요.
    ...
    def run(self):
        """
        접근성 분석을 실행하고 점수를 반환합니다.
        
        Returns:
            float: 접근성 점수 (0-10)
        """
        # 기본 테스트용 프로젝트 정보
        test_project = "웹 접근성을 고려한 스크린 리더 지원과 키보드 네비게이션을 제공하는 프로젝트"
